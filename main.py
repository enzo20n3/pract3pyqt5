...
from PyQt5.QtWidgets import (QApplication, QFileDialog, QHBoxLayout, QLabel, QListView, QMessageBox, QPushButton,
...
class PlaylistModel(QAbstractItemModel):
    ...
    def index(self, row, column, parent=QModelIndex()):
        return self.createIndex(row, column) \
            if self.m_playlist is not None and not parent.isValid() and row >= 0 and row < \
               self.m_playlist.mediaCount() and column >= 0 and column < \
               self.ColumnCount else QModelIndex()
...
class PlayerControls(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.playButton = QToolButton(clicked=self.playClicked, toolTip="Играть")
        self.playButton.setIcon(self.style().standardIcon(QStyle.SP_MediaPlay))
        self.stopButton = QToolButton(clicked=self.stop, toolTip="Стоп")
        self.stopButton.setIcon(self.style().standardIcon(QStyle.SP_MediaStop))
        self.stopButton.setEnabled(False)
        self.nextButton = QToolButton(clicked=self.next, toolTip="Дальше")
        self.nextButton.setIcon(
            self.style().standardIcon(QStyle.SP_MediaSkipForward))
        self.previousButton = QToolButton(clicked=self.previous, toolTip="Предыдущий")
        self.previousButton.setIcon(
            self.style().standardIcon(QStyle.SP_MediaSkipBackward))
        self.muteButton = QToolButton(clicked=self.muteClicked, toolTip="Мут")
...
class Player(QWidget):
    ...
    def __init__(self, playlist, parent=None):
        ...
        openAddButton.setIcon(QIcon.fromTheme("добавить"))
        clearButton = QPushButton("", clicked=self.clearList)
        clearButton.setFixedWidth(36)
        clearButton.setIcon(QIcon.fromTheme("редактировать-удалить"))
        clearButton.setToolTip("Очистить список")
    ...
    def open(self):
        fileNames, _ = QFileDialog.getOpenFileNames(self, "Открыть файл", "", "Audio Files *.mp3 *.m4a *.ogg *.wav *.m3u")
        if fileNames:
            self.url = fileNames
            self.setPlaylist(fileNames)
            print("добавил файлы в плейлист")
    ...
    def statusChanged(self, status):
        ...
        if status == QMediaPlayer.LoadingMedia:
            ...
            self.setStatusInfo("Медиа остановлено")
